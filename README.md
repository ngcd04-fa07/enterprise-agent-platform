...
It's built as a staged, reviewable roadmap (Stage 0 → Stage 21), currently through **Stage 14 of 21** — auth/RBAC through a working end-to-end retrieval UI, hardened with CI and a full-journey integration test, schema-validated structured extraction from a local LLM, hybrid (semantic + lexical) search backed by a real benchmark, a first agentic workflow (automated underwriting triage with deterministic rules, an auditable pipeline, and mandatory human approval), an MCP server exposing all of it as typed tools with zero new trust decisions, and AI call tracing (every LLM/embedding/retrieval call, latency and error rate, independent of the feature that triggered it). Automated agent evaluation is next.
